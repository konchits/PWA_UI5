			...
			actions : {
						...
						success : function (aEntityData) {
							this.waitFor({
								...
								success : function (oList) {
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("Iso_code") === oObject.Iso_code;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].Iso_code;
									} else {
										sCurrentId = aItemsNotInTheList[0].Iso_code;
									}
									...
									oCurrentItem.bindingPath = "/" + oList.getModel().createKey("Europe", {
										Iso_code : sCurrentId
									});
				...
				iRememberTheListItem : function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("Iso_code"),
			...
			assertions : {
				...
				theListHeaderDisplaysZeroHits : function () {
					return this.waitFor({
						...
						id: "masterPageTitle",
						autoWait: false,
						matchers: new PropertyStrictEquals({name : "text", value : "<Europe> (0)"}),
						success: function () {
				...
				theListShouldHaveAllEntries : function () {
					...
					// retrieve all Europe to be able to check for the total amount
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							this.waitFor({
								id : "masterPageTitle",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name : "text", value : "<Europe> (" + iExpectedLength + ")"}),
								success : function () {
